...
- **npm** — manages front-end dependencies (jQuery, jQuery UI, qtip2, Mousetrap, Font Awesome, and the Lato / Merriweather / Crimson Text webfonts via `@fontsource`) and the Dart Sass compiler. Run `npm install && npm run build` once after cloning; `docker-run.sh` does this automatically if any of the built assets are missing. The build step copies JS/CSS/font files from `node_modules/` and compiles `scss/application.scss` → `css/application.css` (Dart Sass). All built files are git-ignored.
- **Webfonts are self-hosted, not fetched from Google.** `scripts/build-assets.js` copies the Latin subsets (regular and bold, upright and italic) into `static/fonts/webfonts/` and generates `scss/dependencies/_webfonts.scss` from the `@font-face` rules that each `@fontsource` package ships, rewriting the URLs. Both outputs are git-ignored and must be regenerated by `npm run build`. To change a typeface, edit the `TYPEFACES` list in the build script and the three `$statedecoded-stack*` variables in `scss/dependencies/variables/_typography.css.scss`.
